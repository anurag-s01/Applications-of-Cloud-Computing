...
void vm_init(struct vm *vm, size_t mem_size)
{
	...
	// printf("%p\n", vm->mem);  //Qn1 Prints the virtual memory address in hypervisor addr space (Q1)
...
void vcpu_init(struct vm *vm, struct vcpu *vcpu)
{
	...
	vcpu->kvm_run = mmap(NULL, vcpu_mmap_size, PROT_READ | PROT_WRITE,
			     MAP_SHARED, vcpu->fd, 0);  //Qn2 here

	// printf("size = %d, addr = %p\n", vcpu_mmap_size, vcpu->kvm_run);  //Qn2
...
int run_vm(struct vm *vm, struct vcpu *vcpu, size_t sz)
{
	...
	for (;;) {
		if (ioctl(vcpu->fd, KVM_RUN, 0) < 0) { //Qn6 Control switch
			perror("KVM_RUN");
			exit(1);
		}
		numExits++;
		...
		case KVM_EXIT_IO:
			...
			{ //open hypercall
				...
				int addr = *(uint32_t *)p;
				char *adr = (char *)vm->mem + addr;
				struct open_query *ptr = (struct open_query *)adr;
				int flags = ptr->flags;
				const char *pathname = ptr->pathname;
				char *act_pathname = (uintptr_t)pathname + (char *)vm->mem;
				mode_t mode = ptr->mode;
				int fd;
				if (ptr->mode_present)
				{
					fd = open(act_pathname, flags, mode);
				}
				else
				{
					fd = open(act_pathname, flags);
				}
				// printf("%d\n", fd);
				ptr->fd = fd;
				// printf("Opened %d\n", fd);
			...
			{ //close hypercall
				...
				int addr = *(uint32_t *)p;
				char *adr = (char *)vm->mem + addr;
				struct close_query *ptr = (struct close_query *)adr;
				int fd = ptr->fd;
				ptr->ret_val = (int)close(fd); //Handle error
				// printf("Closed %d\n", fd);
				continue;
			}
			...
			{ //write hypercall
				...
				int addr = *(uint32_t *)p;
				char *adr = (char *)vm->mem + addr;
				struct write_query *ptr = (struct write_query *)adr;
				const char *buf = (const char *)ptr->buf;
				char *act_buf= (uintptr_t)buf + (char *)vm->mem;
				size_t ret = write(ptr->fd, act_buf, ptr->count);
				ptr->ret_val = ret; //Handle error
				// printf("Closed %d\n", fd);
				continue;
			}
			else if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT && vcpu->kvm_run->io.port == 0xF0)
			{ //read hypercall
				char *p = (char *)vcpu->kvm_run + vcpu->kvm_run->io.data_offset;
				int addr = *(uint32_t *)p;
				char *adr = (char *)vm->mem + addr;
				struct read_query *ptr = (struct read_query *)adr;
				const char *buf = (const char *)ptr->buf;
				char *act_buf = (uintptr_t)buf + (char *)vm->mem;
				size_t ret = read(ptr->fd, act_buf, ptr->count);
				ptr->ret_val = ret; //Handle error
				// printf("Closed %d\n", fd);
				continue;
			}
			else if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT && vcpu->kvm_run->io.port == 0xF1)
			{ //seek hypercall
				char *p = (char *)vcpu->kvm_run + vcpu->kvm_run->io.data_offset;
				int addr = *(uint32_t *)p;
				char *adr = (char *)vm->mem + addr;
				struct seek_query *ptr = (struct seek_query *)adr;
				off_t ret = lseek(ptr->fd, ptr->offset, ptr->whence);
				ptr->ret_val = ret; //Handle error
				// printf("Closed %d\n", fd);
				continue;
			}
...
static void setup_long_mode(struct vm *vm, struct kvm_sregs *sregs)
{
	...
	pd[0] = PDE64_PRESENT | PDE64_RW | PDE64_USER | PDE64_PS;  //Qn3 Page table setup
...
int run_long_mode(struct vm *vm, struct vcpu *vcpu)
{
	...
	/* Clear all FLAGS bits, except bit 1 which is always set. */
	regs.rflags = 2;
	regs.rip = 0;  //Qn5 User entry
	/* Create stack at top of 2 MB page and grow down. */
	regs.rsp = 2 << 20;   //Qn3 Kernel Stack
	...
	memcpy(vm->mem, guest64, guest64_end-guest64);  //Qn3 part 1
